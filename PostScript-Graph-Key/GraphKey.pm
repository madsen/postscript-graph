package PostScript::GraphKey;
use strict;
use warnings;
use PostScript::File 0.04 qw(str);
...
This is a companion object to PostScript::GraphPaper, used by any module that requies a Key for a graph.  It
automatically adjusts size and shape to accomodate the number of items in the space available, adding more columns
...
If given, this should be a PostScript::GraphPaper object.  A GraphKey needs to know which GraphPaper
...
sub build_key {
    ...
    die "No PostScript::GraphPaper object\nStopped" unless (ref($o->{gp}) eq "PostScript::GraphPaper");
...
point.  For example, here is the code used by PostScript::XYChart.  It calculates the mid point of a diagonal line
and draws it using the same functions used for the main chart.  C<line_outer> etc. are provided by
PostScript::GraphStyle to change style settings, C<draw1point> just expects 'x y' and C<drawxyline> expects an
...
L<PostScript::File>, L<PostScript::GraphStyle>, L<PostScript::GraphPaper>.
